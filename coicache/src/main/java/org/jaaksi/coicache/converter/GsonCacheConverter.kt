package org.jaaksi.coicache.converter
...
class GsonCacheConverter(private val gson: Gson) : ICacheConverter {
    ...
    override fun <T> load(source: InputStream, type: Type): CacheEntity<T>? {
        var entity: CacheEntity<T>? = null
        try {
            val adapter = gson.getAdapter(
                TypeToken.getParameterized(CacheEntity::class.java, type)
            ) as TypeAdapter<CacheEntity<T>>
